...
def getHexaValue = (ascii: Char) => ascii.toInt match 
    ...
    case _ => throw new Exception("Incorrect hex character" + ascii + ".")
...
def breakSingleByteXORCipher(input: List[Char]): Int=
    ...
    do
        ...
        for ((key, value) <- freqCharEnglish)
          ...
          for chars <- result
          do 
              if !(freqCharEnglish.contains(chars)) then
                  metric += 1
          if metric < 40 && (metric < bestMetric || bestMetric < 0) then
              bestMetric = metric
              key = byte
    key
...
@main def Challenge(): Unit =
    ...
    for (line <- source.getLines())
        val byte = breakSingleByteXORCipher(line.toList)
        if byte != -1 then
